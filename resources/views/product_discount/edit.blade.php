<x-admin-layout>
    ...
    <div class="mx-auto block max-w-lg rounded-lg bg-white p-6 shadow-4 dark:bg-surface-dark">
        ...
        <form action="{{ route('product-discount.update', $product_discount->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <div class="mt-4 mb-4">
                ...
                <select name="products_id" id="customer_groups_id" class="shadow appearance-none border bg-white rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    ...
                    @foreach ($products as $item)
                        <option value="{{ $item->id }}" @if ($product_discount->products_id == $item->id) selected @endif > {{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <input
                    ...
                    id="quantity" type="text" name="quantity" value="{{ $product_discount->quantity }}" placeholder="Quantity">
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="username">
                    Discount 
                ...
                <input
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="discount" type="text" name="discount_price"  value="{{ $product_discount->discount_price }}" placeholder="Discount Amount">
            </div>
            ...
            <div class="mb-4">
                ...
                <input
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                id="start_date" type="date" name="start_date" value="{{ $product_discount->start_date }}" placeholder="start date">
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="username">
                    End Date
                ...
                <input
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                id="end_date" type="date" name="end_date" value="{{ $product_discount->end_date }}" placeholder="end date">
            </div>
