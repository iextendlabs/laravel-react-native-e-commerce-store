        ...
        <div class="overflow-x-auto sm:-mx-6 lg:-mx-8">
            <form action="{{ route('orders') }}" method="GET">
                ...
                <div class="mb-4">
                    ...
                    <input
                        ...
                        id="name" type="text" name="name" placeholder="Customer Name">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="date">Date</label>
                    <input
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="date" type="date" name="date" value="{{ request('date') }}" placeholder="Date">
                </div>
                <div class="inline-block relative">
                    ...
                    <select
                        ...
                        <option value="Order placed" {{ request('status') === 'Order placed' ? 'selected' : '' }}>Order
                            placed</option>
                        <option value="Pending" {{ request('status') === 'Pending' ? 'selected' : '' }}>Pending</option>
                        <option value="Order confirmed" {{ request('status') === 'Order confirmed' ? 'selected' : '' }}>
                            Order confirmed</option>
                        <option value="Order processing"
                            {{ request('status') === 'Order processing' ? 'selected' : '' }}>Order processing</option>
                        <option value="Shipped" {{ request('status') === 'Shipped' ? 'selected' : '' }}>
                            Shipped/Dispatched</option>
            ...
            <div class="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                ...
                <div class="overflow-hidden">
                    <label for="">Orders:({{ $count }})</label>
                    <table
                        class="min-w-full border-2 border-gray-500 text-left text-sm font-light text-surface mb-20 dark:text-white">
                                    ...
                                    <tr class="border-2 border-neutral-200 dark:border-white/10">
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 flex">
                                            ...
                                            <a href="{{ route('admin.order.detail', $item->id) }}"
                                                class="text-green-500 dark:text-gray-400">view detail</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
